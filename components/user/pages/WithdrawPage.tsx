...
const WithdrawPage: React.FC<WithdrawPageProps> = ({ setCurrentPage }) => {
    ...
    const blockTransaction = (tx: Omit<UserTransaction, 'id' | 'timestamp' | 'verificationMethod'>, reason: string) => {
        addAlert({
            description: `Withdrawal of KES ${tx.amount} at ${tx.recipient} blocked. Reason: ${reason}`,
            riskScore: tx.riskScore,
            relatedTxId: `tx_${Date.now()}`,
        });
        showToast('Withdrawal Blocked! High risk detected.', 'error');
    };
    ...
    return (
        <>
            ...
            <div>
                <PageHeader title={t('withdraw_title')} onBack={() => setCurrentPage('dashboard')} />
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="recipient" className="block text-sm font-medium">{t('withdraw_agent')}</label>
                    ...
                    <button type="submit" className="w-full py-3 bg-emerald-500 text-white rounded-lg font-bold disabled:opacity-50" disabled={!recipient || !amount}>
                        {t('withdraw_button')}
                    </button>
                </form>
            </div>
